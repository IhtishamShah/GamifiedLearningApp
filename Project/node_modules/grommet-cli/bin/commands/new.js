...
exports.default = function (vorpal) {
  ...
  vorpal.command('new [app]', 'Create a new grommet app.\n       Check type option for different ways to initialize an app.').option('-t, --type [type]', 'Type of the generated app (' + config.appTypes.join() + '). Defaults to full.\n      (You can press tab for autocomplete)', config.appTypes).option('--theme [theme]', 'Theme of the generated app (' + config.appThemes.join() + '). Defaults to grommet.\n      (You can press tab for autocomplete)', config.appThemes).option('-d, --description [description]', 'Quick description of the app. Defaults to empty.').option('-r, --repository [repository]', 'Repository URL of the project. Defaults to empty.').option('-l, --license [license]', 'Project license. Defaults to empty.').option('-i, --noInstall', 'Skip npm install after generation is completed. Defaults to false.').validate(function (args) {
        ...
        try {
          (0, _cli.generateProject)(templateFolder, newAppPath, options, config).then(function () {
            ...
            if (options.noInstall) {
              console.log('[' + config.delimiter + '] NPM install has been skipped');
              return Promise.resolve();
            } else {
              return (0, _cli.runNpmInstall)(newAppPath, config);
            }
          }).then(cb);
        } catch (err) {
